...
export class SMSServiceEvery8D implements SMSService<Every8DSMSRequest, Every8DSMSSendResponse, Every8DSMSMultiTargetRequest> {
  ...
  async send(requests: Every8DSMSMultiTargetRequest | Every8DSMSRequest | Every8DSMSRequest[]): Promise<Every8DSMSSendResponse | Every8DSMSSendResponse[]> {
    ...
    const batches = (Array.isArray(requests) ? requests : [requests])
      .reduce<Map<string, string[]>>((batchMap, request) => {
        ...
        if (TAIWAN_PHONE_NUMBER_RE.test((request as Every8DSMSRequest).mobile)) {
          ...
          throw new Error(`${(request as Every8DSMSRequest).mobile} is not taiwan mobile phone (\`onlyTaiwanMobileNumber\` option is true)`);
        }
    ...
    const result = await Array.from(batches.entries())
      .map(([message, mobileList]) => async (resultMap: Map<string, Every8DSMSSendResponse>) => {
        const { data } = await axios
          .post<string>(`${this.baseUrl}/API21/HTTP/SendSMS.ashx`, new URLSearchParams({
            UID: this.username,
            PWD: this.password,
            MSG: message,
      ...
      .reduce<Promise<Map<string, Every8DSMSSendResponse>>>((prev, next) => prev.then(next), Promise.resolve(new Map<string, Every8DSMSSendResponse>()));
