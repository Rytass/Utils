    ...
    >
{
  ...
  constructor(options: Every8DSMSRequestInit) {
    ...
    this.onlyTaiwanMobileNumber =
      options.onlyTaiwanMobileNumber || this.onlyTaiwanMobileNumber;
  }
  ...
  async send(
    request: Every8DSMSMultiTargetRequest,
  ): Promise<Every8DSMSSendResponse[]>;
  ...
  async send(
    ...
    const batches = (Array.isArray(requests) ? requests : [requests]).reduce<
      Map<string, string[]>
    >((batchMap, request) => {
      if ((request as Every8DSMSMultiTargetRequest).mobileList) {
        return (request as Every8DSMSMultiTargetRequest).mobileList.reduce<
          Map<string, string[]>
        >((batch, mobile) => {
          const targetBatch =
            batch.get((request as Every8DSMSMultiTargetRequest).text) || [];
      ...
      const targetBatch =
        batchMap.get((request as Every8DSMSRequest).text) || [];

      if (TAIWAN_PHONE_NUMBER_RE.test((request as Every8DSMSRequest).mobile)) {
        const taiwanMobile = normalizedTaiwanMobilePhoneNumber(
    ...
    const result = await Array.from(batches.entries())
      .map(
        ([message, mobileList]) =>
          async (resultMap: Map<string, Every8DSMSSendResponse>) => {
            ...
            if (batchId) {
              return mobileList.reduce((map, mobile, index) => {
                ...
                map.set(`${message}:${mobile}`, {
                  messageId: batchId,
                  status: sent
                    ? SMSRequestResult.SUCCESS
                    : SMSRequestResult.FAILED,
                  mobile,
    ...
    if ((requests as Every8DSMSMultiTargetRequest).mobileList) {
      return (requests as Every8DSMSMultiTargetRequest).mobileList.map(
        (mobile) => {
          ...
          return result.get(
            `${(requests as Every8DSMSMultiTargetRequest).text}:${sendMobile}`,
          )!;
        },
      );
    }
