...
describe('ConverterManager', () => {
  ...
  convert1.mockImplementation(async (file: ConvertableFile): Promise<Buffer> => {
    return Buffer.concat([file as Buffer, Buffer.from([0x01])]);
  });

  convert2.mockImplementation(async (file: ConvertableFile): Promise<Buffer> => {
  ...
  it('should convert method can iterator its converters', (done) => {
    converterManager.convert(Buffer.from([0x00])).then((resultFile) => {
      expect(convert1).toBeCalledTimes(1);
      expect(convert2).toBeCalledTimes(1);

      expect(Buffer.compare(resultFile as Buffer, Buffer.from([0x00, 0x01, 0x02]))).toBe(0);
