...
export const OptionProviders = [
  {
    ...
    useFactory: async (options?: CMSGraphqlBaseModuleOptionsDto) =>
      options?.mapArticleCustomFieldsToEntityColumns ? options.mapArticleCustomFieldsToEntityColumns : () => ({}),
    inject: [CMS_BASE_GRAPHQL_MODULE_OPTIONS],
  },
  {
    provide: MAP_CATEGORY_CUSTOM_FIELDS_TO_ENTITY_COLUMNS,
    useFactory: async (options?: CMSGraphqlBaseModuleOptionsDto) =>
      options?.mapCategoryCustomFieldsToEntityColumns ? options.mapCategoryCustomFieldsToEntityColumns : () => ({}),
    inject: [CMS_BASE_GRAPHQL_MODULE_OPTIONS],
  },
] as Provider[];
