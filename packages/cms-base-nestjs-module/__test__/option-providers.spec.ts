...
describe('OptionProviders', () => {
  ...
  it('should return correct values for boolean feature flags', () => {
    ...
    expect(getFactory(MULTIPLE_CATEGORY_PARENT_MODE)?.(mockDefaultOptions)).toBe(true);
    ...
    expect(getFactory(AUTO_RELEASE_AFTER_APPROVED)?.(mockDefaultOptions)).toBe(true);
  });
  ...
  it('should fallback to defaults if options are missing', async () => {
    ...
    expect(getFactory(SIGNATURE_LEVELS)?.({})).toEqual([DEFAULT_SIGNATURE_LEVEL]);
  ...
  it('should return provided values for entity providers', () => {
    expect(getFactory(PROVIDE_ARTICLE_ENTITY)?.(mockDefaultOptions)).toBe('ArticleEntity');

    expect(getFactory(PROVIDE_ARTICLE_VERSION_ENTITY)?.(mockDefaultOptions)).toBe('VersionEntity');

    expect(getFactory(PROVIDE_ARTICLE_VERSION_CONTENT_ENTITY)?.(mockDefaultOptions)).toBe('ContentEntity');

    expect(getFactory(PROVIDE_CATEGORY_ENTITY)?.(mockDefaultOptions)).toBe('CategoryEntity');

    expect(getFactory(PROVIDE_CATEGORY_MULTI_LANGUAGE_NAME_ENTITY)?.(mockDefaultOptions)).toBe('CategoryNameEntity');

    expect(getFactory(PROVIDE_SIGNATURE_LEVEL_ENTITY)?.(mockDefaultOptions)).toBe('SignatureEntity');
  });
  ...
  it('should fallback to null when entity options are undefined', () => {
    ...
    expect(getFactory(PROVIDE_ARTICLE_VERSION_ENTITY)?.(emptyOptions)).toBeNull();

    expect(getFactory(PROVIDE_ARTICLE_VERSION_CONTENT_ENTITY)?.(emptyOptions)).toBeNull();

    expect(getFactory(PROVIDE_CATEGORY_ENTITY)?.(emptyOptions)).toBeNull();
    expect(getFactory(PROVIDE_CATEGORY_MULTI_LANGUAGE_NAME_ENTITY)?.(emptyOptions)).toBeNull();

    expect(getFactory(PROVIDE_SIGNATURE_LEVEL_ENTITY)?.(emptyOptions)).toBeNull();
  });
