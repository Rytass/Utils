...
export class TCatLogisticsService<T extends TCatLogisticsInterface<LogisticsStatus<T>>> implements LogisticsService<T> {
  ...
  private getLogisticsStatuses(id: string, html: string): TCatLogisticsTraceResponse<T> {
    const statusHistory = this.configuration.statusMap(html, id) as ReturnType<T['statusMap']>;

    if (!statusHistory.length && !this.configuration.ignoreNotFound)
      throw new LogisticsError(ErrorCode.NOT_FOUND_ERROR, `ID:${id} is not found.`);
  ...
  async trace(logisticsIds: string | string[]): Promise<LogisticsTraceResponse<T>[]> {
    ...
    const ids: string[] = typeof logisticsIds === 'string' ? [logisticsIds] : logisticsIds;
