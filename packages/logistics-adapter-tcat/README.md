...
const multipleResults = await logisticsService.trace([
  '800978442950', 
  '903404283301',
  '123456789012'
]);
...
class LogisticsDashboard {
  ...
  async getTrackingSummary(trackingNumbers: string[]) {
    ...
    results.forEach(result => {
      ...
      if (latestStatus) {
        // Count by status
        summary.byStatus[latestStatus.status] = 
          (summary.byStatus[latestStatus.status] || 0) + 1;
        ...
        if (latestStatus.status === 'DELIVERY_ATTEMPTED' || 
            latestStatus.status === 'RETURNED_TO_SENDER') {
          summary.issues.push({
            logisticsId: result.logisticsId,
  ...
  async getDeliveryReport(trackingNumbers: string[], dateRange: { from: Date; to: Date }) {
    ...
    results.forEach(result => {
      ...
      } else if (latestStatus.status === 'IN_TRANSIT' || 
                 latestStatus.status === 'OUT_FOR_DELIVERY') {
        report.inTransit++;
      } else {
        report.issues++;
      }
...
class ScheduledTrackingService {
  ...
  private async updateShipmentStatus(shipment: any, trackingResult: any) {
    ...
    if (shipment.currentStatus !== latestStatus.status) {
      ...
      if (latestStatus.status === 'DELIVERED' || 
          latestStatus.status === 'DELIVERY_ATTEMPTED') {
