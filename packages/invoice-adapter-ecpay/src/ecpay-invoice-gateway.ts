import { CustomsMark, getTaxTypeFromItems, InvoiceAllowanceState, InvoiceCarrierType, InvoiceGateway, InvoiceMobileCarrier, InvoiceMoicaCarrier, TaxType } from '@rytass/invoice';
...
export class ECPayInvoiceGateway implements InvoiceGateway<ECPayInvoice, ECPayInvoiceQueryOptions> {
  ...
  async allowance(invoice: ECPayInvoice, allowanceItems: ECPayPaymentItem[], options?: ECPayInvoiceAllowanceOptions): Promise<ECPayInvoice> {
  ...
  private async getInvoiceListInPage(options: ECPayInvoiceListQueryOptions, page: number): Promise<ECPayInvoice[]> {
    ...
    const { data } = await axios.post<ECPayQueryListInvoiceResponse>(`${this.baseUrl}/B2CInvoice/GetIssueList`, JSON.stringify({
      MerchantID: this.merchantId,
      RqHeader: {
        Timestamp: now,
      },
    ...
    if (data.TransCode === ECPAY_INVOICE_SUCCESS_CODE) {
      return data.Data.InvoiceData.map(invoice => new ECPayInvoice({
        items: [{
