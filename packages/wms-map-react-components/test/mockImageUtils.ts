...
export const generateMockImageUrl = (filename: string): string => {
  ...
  if (imageMappings[filename]) {
    finalUrl = imageMappings[filename];
  } else {
    ...
    finalUrl = `https://via.placeholder.com/${width}x${height}/${bgColor}/${textColor}?text=${text}`;
  }
