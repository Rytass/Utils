...
import { mockMapData, simpleMapData, largeMapData } from '../src/utils/mockData';
...
const MockDataTest: React.FC = () => {
  ...
  const loadTestData = (type: 'simple' | 'complex' | 'large') => {
    ...
    try {
      ...
      console.error(`❌ 載入 ${typeName} 失敗:`, error);
      alert(`載入失敗: ${error}`);
    }
  };
    ...
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      {currentNodes.length > 0 && (
        <div style={{ 
          ...
          <h3>📊 當前載入的資料統計</h3>
          <p><strong>測試類型:</strong> {currentTestType}</p>
          <p><strong>節點總數:</strong> {currentNodes.length}</p>
          <p><strong>背景圖片:</strong> {currentNodes.filter(n => n.type === 'imageNode').length}</p>
          <p><strong>矩形區域:</strong> {currentNodes.filter(n => n.type === 'rectangleNode').length}</p>
          <p><strong>多邊形區域:</strong> {currentNodes.filter(n => n.type === 'pathNode').length}</p>
      ...
      <div style={{ 
        ...
        <h3>💡 測試說明</h3>
        <ul>
          <li><strong>簡單測試資料:</strong> 適合基本功能驗證，包含最基本的元素</li>
          <li><strong>完整倉庫資料:</strong> 模擬真實倉庫的複雜佈局，包含多種形狀和區域</li>
          <li><strong>大量測試資料:</strong> 用於性能測試，包含大量節點</li>
          <li><strong>檢視模式:</strong> Modal 預設為檢視模式，可以測試底圖顯示切換功能</li>
          <li><strong>點擊測試:</strong> 點擊任何區域或背景，會在 console 輸出點擊資訊</li>
        </ul>
      </div>
      ...
      {isModalOpen && (
        <WmsMapModal
          ...
          colorPalette={['#3b82f6', '#ef4444', '#22c55e', '#f59e0b', '#8b5cf6', '#06b6d4']}
