...
export const useDirectStateHistory = ({
  ...
  // 初始化歷史記錄
  const initializeHistory = useCallback((initialNodes: Node[], initialEdges: Edge[]) => {
  ...
  // 保存狀態快照
  const saveState = useCallback((nodes: Node[], edges: Edge[], operation: string) => {
  ...
  const getHistorySummary = useCallback(() => {
    return {
      ...
      operations: debugMode ? history.map((h, index) => ({
        index,
