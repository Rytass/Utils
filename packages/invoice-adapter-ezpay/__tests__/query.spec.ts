...
describe('EZPay Invoice Query', () => {
      ...
      return {
        data: {
          ...
          Result: JSON.stringify({
            ...
            CheckCode: createHash('sha256')
              .update(
                `HashIV=${AES_IV}&InvoiceTransNo=23020318530290616&MerchantID=${MERCHANT_ID}&MerchantOrderNo=90h31g023476g234g&RandomNum=1440&TotalAmt=50&HashKey=${AES_KEY}`,
              )
              .digest('hex')
              .toUpperCase(),
            CreateStatusTime: '',
  ...
  it('should throw on check code invalid', async () => {
      ...
      return {
        data: {
          ...
          Result: JSON.stringify({
            ...
            CheckCode: createHash('sha256')
              .update(
                `1HashIV=${AES_IV}&InvoiceTransNo=23020318530290616&MerchantID=${MERCHANT_ID}&MerchantOrderNo=90h31g023476g234g&RandomNum=1440&TotalAmt=20&HashKey=${AES_KEY}`,
              )
              .digest('hex')
              .toUpperCase(),
            CreateStatusTime: '',
    ...
    ).rejects.toThrow('Invalid CheckCode');
  });
  ...
  it('should throw on item parse failed', async () => {
    ...
    ).rejects.toThrow('Item Parse Failed');
  });
