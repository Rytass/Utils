import { ErrorCode, StorageError, InputFile, StorageFile, Storage, WriteFileOptions, ReadBufferFileOptions, ReadStreamFileOptions } from '@rytass/storages';
...
export class StorageAzureBlobService extends Storage<AzureBlobOptions> {
  ...
  async url(key: string, expires = Date.now() + 1000 * 60 * 60 * 24): Promise<string> {
    ...
    }, blobClient.credential as StorageSharedKeyCredential).toString();
