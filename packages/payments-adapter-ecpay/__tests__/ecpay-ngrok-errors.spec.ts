...
/**
 * Tests for ngrok error handling in ECPayPayment.
...
describe('ECPay Payment Ngrok Error Handling', () => {
  ...
  describe('NGROK_AUTHTOKEN not set', () => {
    it('should throw error when NGROK_AUTHTOKEN is not set', async () => {
      ...
      await expect(listenCallback!()).rejects.toThrow('[ECPayPayment] NGROK_AUTHTOKEN is not set');
    });
  });
