...
| `removePolicy(policy: Policy \| string)`     | this              | Remove policy instance(s) from **builder.policies** based on instance reference or **policy.id** (can active as `builder.hasBuiltOrders` is `false`) |
| `removePolicy(policies: (Policy \| string)[])` | | |
...
| `removeItem(item: OrderItem \| string)`           |                   | |
| `removeItem(items: (OrderItem \| string)[])`        |                   | |
