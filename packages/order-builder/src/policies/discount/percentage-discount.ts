...
export class PercentageDiscount implements BaseDiscount {
  ...
  constructor(value: number, arg1?: DiscountOptions | Condition | Condition[], arg2?: DiscountOptions) {
  ...
  discount(price: number): number {
    return this.options?.excludedInCalculation ? 0 : times(price, minus(1, this.value));
  }
  ...
  description(order: Order, itemValue: number, appliedItems: FlattenOrderItem[]): PolicyDiscountDescription {
    return {
      ...
      value: this.value,
      type: this.type,
      discount: order.config.roundStrategy.round(this.discount(itemValue), ['final-price-only', 'every-calculation']),
