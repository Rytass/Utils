...
export class StepPercentageDiscount implements BaseDiscount {
  ...
  discount(itemValue: number, multiplier: number): number {
    ...
    const discountRate = pow(this.value, Math.min(this.stepLimit, Math.floor(divided(multiplier, this.step))));
  ...
  description(
    ...
    const matchedTimes = Math.min(this.stepLimit, Math.floor(divided(multiplier, this.step)));
    ...
    return {
      ...
      value: this.value,
      type: this.type,
      discount: order.config.roundStrategy.round(this.discount(itemValue, multiplier), [
        'final-price-only',
        'every-calculation',
      ]),
  ...
  resolve<PolicyDiscountDescription>(order: Order, policies: PolicyDiscountDescription[]): PolicyDiscountDescription[] {
    if (this.valid(order)) {
      ...
      policies.push(this.description(order, itemValue, multiplier, matchedItems) as PolicyDiscountDescription);
    }
