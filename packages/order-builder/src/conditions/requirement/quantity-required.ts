  ...
  implements Condition<RequirementDescription<QuantityRequiredInput>, Options>
{
   ...
   */
  constructor(leastQuantity: number, specifiedItems: (QuantityRequiredInput | string)[], options?: Options);
  /**
   * Quantity requirement condition
   * @description To check whether an order has reached given quantity items.
   * @param {Number} leastQuantity Number - A least quantity of items to satisfy condition.
  ...
  constructor(leastQuantity: number, arg1?: (QuantityRequiredInput | string)[] | Options, arg2?: Options) {
  ...
  satisfy(order: Order): boolean {
    ...
    return toCheckItems.reduce((totalQuantity, item) => totalQuantity + item.quantity, 0) >= this.leastQuantity;
  }
}
